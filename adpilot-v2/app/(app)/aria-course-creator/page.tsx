  ...
  return (
    <div className="max-w-3xl animate-fade-in">
      <PageHeader
        ...
        subtitle="Turn your expertise into a sellable course outline — promise, modules, lessons, and a lead magnet."
      />
      ...
      {out && (
        <div className="mt-5 space-y-4">
          <div className="rounded-2xl border border-border-subtle bg-surface-raised p-5 shadow-card">
            <h3 className="mb-1 font-bold text-ink">Course promise</h3><p className="text-sm">{out.promise}</p>
          </div>
          {out.modules.map((m) => (
            <div key={m.t} className="rounded-2xl border border-border-subtle bg-surface-raised p-5 shadow-card">
              <h3 className="mb-2 font-bold text-ink">{m.t}</h3>
              <ul className="list-disc space-y-1 pl-5 text-sm">{m.lessons.map((l, i) => <li key={i}>{l}</li>)}</ul>
            </div>
          ))}
          <div className="rounded-2xl border border-border-subtle bg-surface p-5"><p className="text-sm font-semibold">{out.magnet}</p></div>
