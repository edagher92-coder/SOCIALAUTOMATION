...
export default function Agency() {
  ...
  return (
    <div className="max-w-xl">
      <h1 className="text-2xl font-extrabold tracking-tight">White-label / Agency</h1>
      <p className="mb-5 mt-1 text-muted">Brand the client-facing reports as your own agency.</p>
      ...
      <div className="space-y-4 rounded-2xl border border-[#e3e8ef] bg-white p-5 shadow-card">
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <input type="color" value={color} onChange={(e) => setColor(e.target.value)} className="h-10 w-12 rounded border border-[#e3e8ef]" />
            <input value={color} onChange={(e) => setColor(e.target.value)} className="w-full rounded-lg border border-[#e3e8ef] p-2.5" />
          </div>
        </div>
...
function Field({ label, v, set, ph }: { label: string; v: string; set: (s: string) => void; ph?: string }) {
  return (<div><label className="mb-1 block text-sm font-bold">{label}</label>
    <input value={v} onChange={(e) => set(e.target.value)} placeholder={ph} className="w-full rounded-lg border border-[#e3e8ef] p-2.5" /></div>);
}
