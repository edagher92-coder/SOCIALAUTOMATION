...
const C = {
  ...
  coral: "#f9603f", coralDk: "#e84b2c", coralHot: "#ff7a4d", amber: "#ffb224", amberHot: "#ffc24a",
  sky: "#2fb6c4", skyDeep: "#1f8f9e", greenBright: "#22c55e",
...
// richer 3-stop "hero" gradient for result numbers / CTA / logo (glows instead of fading).
function gradHero(x, y, w, h) { const g = ctx.createLinearGradient(x, y, x + w, y + h); g.addColorStop(0, C.coral); g.addColorStop(0.45, C.coralHot); g.addColorStop(1, C.amberHot); return g; }
function cardGlow(cx, cy, r, rgba) { ctx.save(); const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, r); g.addColorStop(0, rgba); g.addColorStop(1, "rgba(0,0,0,0)"); ctx.fillStyle = g; ctx.fillRect(0, 0, W, H); ctx.restore(); }
// brief, tasteful celebration burst (a flash of joy — used only on the green winner).
function confetti(cx, cy, lt, t0) {
  const e = lt - t0; if (e < 0 || e > 1.1) return; ctx.save();
  const cols = [C.greenBright, C.amber, C.coralHot, C.sky, C.greenBright, C.amber]; let s = 99;
  const rnd = () => { s = (s * 1103515245 + 12345) & 0x7fffffff; return s / 0x7fffffff; };
  for (let i = 0; i < 18; i++) { const a = rnd() * Math.PI * 2, v = 280 + rnd() * 360; const vx = Math.cos(a) * v, vy = Math.sin(a) * v - 240; const x = cx + vx * e, y = cy + vy * e + 540 * e * e; const life = clamp(1 - e / 1.0); ctx.globalAlpha = life * 0.9; ctx.fillStyle = cols[i % cols.length]; ctx.save(); ctx.translate(x, y); ctx.rotate(e * (4 + rnd() * 6)); rr(-7, -4, 14, 8, 3); ctx.fill(); ctx.restore(); }
  ctx.restore();
}
// friendly rounded vector icons (real emoji don't render in canvas): thumb / warn / rocket / clock
function fIcon(key, x, y, s, col) {
  ctx.save(); ctx.strokeStyle = col; ctx.fillStyle = col; ctx.lineWidth = s * 0.13; ctx.lineCap = "round"; ctx.lineJoin = "round";
  if (key === "thumb") { rr(x - s * 0.42, y - s * 0.05, s * 0.28, s * 0.5, s * 0.06); ctx.fill(); ctx.beginPath(); ctx.moveTo(x - s * 0.1, y + s * 0.42); ctx.lineTo(x - s * 0.1, y - s * 0.06); ctx.lineTo(x + s * 0.16, y - s * 0.34); ctx.arcTo(x + s * 0.28, y - s * 0.34, x + s * 0.22, y - s * 0.08, s * 0.12); ctx.lineTo(x + s * 0.12, y - s * 0.04); ctx.lineTo(x + s * 0.42, y - s * 0.04); ctx.arcTo(x + s * 0.5, y - s * 0.04, x + s * 0.46, y + s * 0.12, s * 0.08); ctx.lineTo(x + s * 0.36, y + s * 0.42); ctx.lineTo(x - s * 0.1, y + s * 0.42); ctx.closePath(); ctx.fill(); }
  else if (key === "warn") { ctx.beginPath(); ctx.moveTo(x, y - s * 0.42); ctx.lineTo(x + s * 0.46, y + s * 0.38); ctx.lineTo(x - s * 0.46, y + s * 0.38); ctx.closePath(); ctx.lineWidth = s * 0.1; ctx.stroke(); ctx.lineWidth = s * 0.13; ctx.beginPath(); ctx.moveTo(x, y - s * 0.12); ctx.lineTo(x, y + s * 0.12); ctx.stroke(); ctx.beginPath(); ctx.arc(x, y + s * 0.26, s * 0.04, 0, Math.PI * 2); ctx.fill(); }
  else if (key === "rocket") { ctx.beginPath(); ctx.moveTo(x, y - s * 0.45); ctx.arcTo(x + s * 0.24, y - s * 0.1, x + s * 0.16, y + s * 0.22, s * 0.3); ctx.lineTo(x - s * 0.16, y + s * 0.22); ctx.arcTo(x - s * 0.24, y - s * 0.1, x, y - s * 0.45, s * 0.3); ctx.closePath(); ctx.fill(); ctx.fillStyle = "#fff"; ctx.beginPath(); ctx.arc(x, y - s * 0.1, s * 0.08, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = C.amber; ctx.beginPath(); ctx.moveTo(x - s * 0.1, y + s * 0.22); ctx.lineTo(x, y + s * 0.46); ctx.lineTo(x + s * 0.1, y + s * 0.22); ctx.closePath(); ctx.fill(); }
  else if (key === "clock") { ctx.lineWidth = s * 0.1; ctx.beginPath(); ctx.arc(x, y, s * 0.4, 0, Math.PI * 2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x, y - s * 0.24); ctx.moveTo(x, y); ctx.lineTo(x + s * 0.18, y + s * 0.06); ctx.stroke(); }
  ctx.restore();
}
...
function meshStage(lt) {
  ...
  glow(170 + 22 * Math.sin(lt * 0.18), 300 + 14 * Math.cos(lt * 0.15), 740, "rgba(249,96,63,0.14)");
  glow(910 - 16 * Math.cos(lt * 0.13), 360 + 18 * Math.sin(lt * 0.12), 800, "rgba(255,178,36,0.16)");
  glow(840 + 14 * Math.sin(lt * 0.1), 1180 + 16 * Math.cos(lt * 0.09), 700, "rgba(47,182,196,0.07)");
  glow(540, 1560, 820, "rgba(249,96,63,0.06)");
  ctx.save(); const tg = ctx.createLinearGradient(0, 0, 0, 420); tg.addColorStop(0, "rgba(255,178,36,0.05)"); tg.addColorStop(1, "rgba(255,178,36,0)"); ctx.fillStyle = tg; ctx.fillRect(0, 0, W, 420); ctx.restore();
}
...
function s2(lt) { // Bean & Bloom diagnosis — 58 Orange (LIGHT product world, gridded)
  ...
  ctx.globalAlpha = clamp(seg(lt, 0.04, 0.3) * 2); ctx.fillStyle = C.ink; ctx.font = `60px ${DISP}`; ctx.textAlign = "center"; ctx.textBaseline = "alphabetic"; ctx.fillText("YOUR ADS' REPORT CARD", 540, 412); ctx.globalAlpha = 1;
  cardGlow(540, 880, 620, "rgba(234,88,12,0.10)");
  ...
  if (cardP > 0.5) {
    ...
    ctx.fillStyle = C.mutedL; ctx.font = `27px ${UI5}`; ctx.fillText("A few ads are quietly leaking money.", 540, 1322); ctx.globalAlpha = 1;
  }
...
function s6(lt) { // Account 2 — Coach Maya 80 Green (LIGHT, mirrors s2 exactly)
  meshStage(lt);
  ctx.globalAlpha = clamp(seg(lt, 0.03, 0.25) * 2); ctx.fillStyle = C.ink; ctx.font = `54px ${DISP}`; ctx.textAlign = "center"; ctx.textBaseline = "alphabetic"; ctx.fillText("WHEN IT'S ALL WORKING", 540, 232); ctx.globalAlpha = 1;
  eyebrow("A HEALTHY ACCOUNT · DEMO", 300, C.mutedL);
  cardGlow(540, 880, 660, "rgba(34,197,94,0.16)");
  const cardP = easeOutCubic(clamp(seg(lt, 0.12, 0.42)));
  ctx.save(); ctx.globalAlpha = cardP; ctx.translate(540, 980); ctx.scale(lerp(0.97, 1, cardP), lerp(0.97, 1, cardP)); ctx.translate(-540, -980);
  shadowCard(96, 480, 888, 990, 28, true); ctx.restore();
  if (cardP > 0.5) {
    ...
    ctx.globalAlpha = clamp(seg(lt, 1.05, 0.4) * 2); ctx.fillStyle = C.ink; ctx.font = `36px ${UI6}`; ctx.textAlign = "center"; ctx.textBaseline = "alphabetic"; ctx.fillText("Looking great — your money's working.", 540, 1268);
    ctx.fillStyle = C.mutedL; ctx.font = `27px ${UI5}`; ctx.fillText("Every $1 spent made $2.19 back", 540, 1322); ctx.globalAlpha = 1;
  }
  confetti(540, 850, lt, 1.0);
  captionPill([{ t: "Looking great — " }, { t: "do more of this.", pill: C.green }], 1700, seg(lt, 1.25, 0.4), false); demoTag(false);
}
...
function s8(lt) { // 12 specialists (LIGHT product world, diagonal-wavefront stagger)
  meshStage(lt);
  ctx.globalAlpha = clamp(seg(lt, 0.05, 0.3) * 2); ctx.textAlign = "center"; ctx.textBaseline = "alphabetic"; richLine([{ t: "YOUR ", c: C.ink }, { t: "AI ADS TEAM", c: C.coral }], 540, 360, `58px ${DISP}`); ctx.fillStyle = C.mutedL; ctx.font = `30px ${UI6}`; ctx.fillText("a specialist for every job — in plain English", 540, 410); ctx.globalAlpha = 1;
...
function s9(lt) { // plan tiers (surface light)
  ...
  captionPill([{ t: "Free to start. " }, { t: "Cancel anytime.", pill: C.coral }], 1700, seg(lt, 0.7, 0.4), false);
}
...
// Plain-English "what to do" — 4 relatable demo cards a non-marketer instantly gets.
function sDemo(lt) {
  meshStage(lt);
  ctx.globalAlpha = clamp(seg(lt, 0.03, 0.25) * 2); eyebrow("REAL ADS · PLAIN-ENGLISH ADVICE", 252, C.mutedL);
  text("IT TELLS YOU WHAT TO DO", 540, 372, `52px ${DISP}`, C.ink, "center"); ctx.globalAlpha = 1;
  const cards = [
    { icon: "warn", ic: C.red, tint: "rgba(220,38,38,0.12)", name: "“Winter Menu” ad", money: "Spent $1,250 · made $0 back", rec: "Turn it off", rc: C.red },
    { icon: "rocket", ic: C.green, tint: "rgba(34,197,94,0.14)", name: "“Brunch Reel”", money: "Every $1 made $2.40 back", rec: "Put more here", rc: C.green },
    { icon: "warn", ic: C.orange, tint: "rgba(234,88,12,0.12)", name: "“20% Off Weekend”", money: "Spent $32 to make a $21 sale", rec: "Fix the offer", rc: C.orange },
    { icon: "clock", ic: C.mutedL, tint: "rgba(107,100,120,0.12)", name: "“New Timetable”", money: "Too few clicks to tell yet", rec: "Give it a few days", rc: C.mutedL },
  ];
  const x = 96, w = 888, h = 196, gap = 20; let y = 470;
  for (let i = 0; i < cards.length; i++) { const c = cards[i]; const p = seg(lt, 0.18 + i * 0.16, 0.4); const e = easeOutBack(clamp(p));
    if (e > 0) { ctx.save(); ctx.globalAlpha = clamp(p * 1.5); ctx.translate(540, y + h / 2 + (1 - clamp(e)) * 28); ctx.scale(lerp(0.94, 1, clamp(e)), lerp(0.94, 1, clamp(e))); ctx.translate(-540, -(y + h / 2));
      shadowCard(x, y, w, h, 26);
      ctx.fillStyle = c.tint; rr(x + 26, y + h / 2 - 44, 88, 88, 24); ctx.fill(); fIcon(c.icon, x + 26 + 44, y + h / 2, 50, c.ic);
      text(c.name, x + 142, y + h / 2 - 8, `36px ${UI8}`, C.ink, "left"); text(c.money, x + 142, y + h / 2 + 34, `26px ${UI5}`, C.mutedL, "left");
      ctx.font = `27px ${UI8}`; const rw = ctx.measureText(c.rec).width + 52, rx = x + w - rw - 26; ctx.fillStyle = c.rc; rr(rx, y + h / 2 - 30, rw, 60, 30); ctx.fill(); text(c.rec, rx + rw / 2, y + h / 2 + 1, `27px ${UI8}`, C.white, "center", "middle");
      ctx.restore(); }
    if (i === 1) confetti(x + 70, y + 30, lt, 0.66);
    y += h + gap;
  }
  captionPill([{ t: "No jargon — just " }, { t: "what to do.", pill: C.coral }], 1700, seg(lt, 1.05, 0.4), false); demoTag(false);
}

// V5 lineup (simpler, demo-led): live dashboard → report-card score → plain-English demos →
// approve → read-only → the green winner → your AI team → pricing → CTA. Light product world.
const SCENES = [[s0, 10.0], [s2, 4.6], [sDemo, 6.2], [s4, 3.6], [s5, 4.2], [s6, 4.6], [s8, 4.2], [s9, 4.6], [s11, 4.8]];
