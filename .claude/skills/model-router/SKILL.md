...
name: model-router
description: >-
  Quality-first algorithm for choosing the Claude model (Haiku 4.5 / Sonnet 4.6 / Opus 4.8 / Fable 5),
...
1. STAKES GATE (overrides everything below):
   ...
   → Opus 4.8, effort ≥ high, and VERIFY. Do not downshift. Stop here.
...
3. SCORE complexity + ambiguity + output type:
   ...
   hard | ambiguous-important | long-horizon → Opus 4.8, effort high–xhigh
...
- **Fast mode is speed, not savings.** `/fast` is the same Opus 4.8 at premium price; use it only when the
